...
# class VectorStore:
#     def __init__(self, embeddings_path: str, document_loader: DocumentLoader, embedding_client: EmbeddingClient):
...
#         similarities /= (np.linalg.norm(self.embeddings_matrix, axis=1) * np.linalg.norm(query_embedding))
        
#         # Get top-k indices
#         print("Getting top-k indices...")
#         top_k_indices = np.argsort(similarities)[-k*2:][::-1]  # Get more than k to account for both abstract and conclusions
...
# results = vector_store.search("Why do older galaxies tend to have higher metallicities?", k=3)
