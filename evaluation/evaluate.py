...
class Evaluator:
    def __init__(self, retrieval_system: RetrievalSystem, system_name: str):
    ...
    def _evaluate_multi_document(self, ground_truth_file: str, top_k: int = 10) -> Dict[str, float]:
        ground_truth = self._load_ground_truth(ground_truth_file)
        results = defaultdict(list)
    ...
    def _map_arxiv_to_filenames(self, arxiv_ids: List[str]) -> List[str]:
        ...
        return [f"arxiv/{arxiv_id[:4]}/{arxiv_id}_arXiv.txt" for arxiv_id in arxiv_ids]
